...
export default {
  ...
  methods: {
      ...
      this.splinechart.off('tooltipchange').on('tooltipchange', (ev) => {
        for (let i = 0; i < ev.items.length; i++) {
          if (ev.items[i].name !== 'TCP连接' && ev.items[i].name !== 'UDP连接') {
            ev.items[i].value = ev.items[i].value + ' Mbps'
          }
        }
      })
    ...
    drawSpline (chart, data) { 
      ...
      this.splinechart.area().position('timestamp*value').color('type', ['rgb(124, 181, 236)', '#000', '#eb9e05', '#9AD681']).size(2)
      this.splinechart.area().position('timestamp*num').color('type', ['#eb9e05', '#9AD681']).size(2)
      chart.render()
    }
  }
}
