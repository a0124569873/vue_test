    ...
    <div id="globalconfig">
        <el-row>
            <el-col :span="24" style="position: relative;">
                <h2 class="config-title" style="padding-bottom: 40px;">
                    汇聚网卡列表
                </h2>
                <div style="position: absolute; top: 0; right: 0">
                    <el-button type="primary" @click="showEthGrpAddDialog">添 加</el-button>
                    <el-button type="primary" @click="handleRemoveEthGrp">删 除</el-button>
                </div>
                <el-table
                    border
                    ...
                    <el-table-column label="IP地址">
                        <template slot-scope="scop">
                            <template v-if="scop.row.ips.length">
                                <div v-for="(ip, index) in scop.row.ips" :key="index">
                                    {{ip}}
                                </div>
                            </template>
                            <template v-else>
                    ...
                    <el-table-column label="绑定网卡">
                        <template slot-scope="scop">
                            <template v-if="scop.row.binds.length">
                                <div v-for="(bind, index) in scop.row.binds" :key="index">
                    ...
                    <el-table-column label="负载均衡模式" prop="load_mode">
                        <template slot-scope="scop">
                            {{load_balance_mode_options_text[scop.row.load_mode]}}
                        </template>
                    </el-table-column>
                    <el-table-column label="操作">
                        <template slot-scope="scop">
                            <el-button @click="showEthGrpEditDialog(scop.row)">配置</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </el-col>
            <el-dialog 
                ...
                <el-row style="margin-bottom: 15px">
                    <el-alert title="IP只能为掩码格式" type="info" :closable="false" show-icon>
                    </el-alert>
                </el-row>
                <div
                ...
                <el-form
                  ...
                  >
                    ...
                    <el-button type="primary" @click.prevent="ethgrpRemoveIpInput(item)">删 除</el-button>
                  </el-form-item>
                  <div style="margin-top: 0px; margin-bottom: 15px; margin-left: 100px; width: 200px">
                    <el-button type="primary" style="width: 100%" @click="ethgrpAddIpInput">增加 ip 配置</el-button>
                  </div>
                  <el-form-item 
                    label="Mac地址"
                    prop="mac">
                  ...
                  <div style="margin-bottom: 30px; padding: 0 5px; font-size: 14px; color: #5a5e66">
                    <h2 style="padding-bottom: 5px; font-size: 14px; border-bottom: 1px solid; color: #5a5e66">绑 定</h2>
                    <div style="display: flex; justify-content: space-around;">
                      ...
                      <div style="display: flex; flex-direction: column; justify-content: center; width: 56px">
                        <el-button @click="addFromAllEth">增 加</el-button>
                        <el-button style="margin: 15px 0 0 0" @click="removeFromEthMode">删 除</el-button>
                      </div>
                      ...
                      <el-table
                        ...
                        max-height="300">
                        <el-table-column prop="name" label="网 卡"></el-table-column>
                        <el-table-column prop="mode" label="模 式">
...
export default {
  ...
  mounted () {
	...
	this.getEthNet()
    this.getMgpp()
    console.log(this.ethData.ethgrpData)
  },
  methods: {
    getMgpp(){
    ...
    submitEthGrpForm (formName, type) {
      ...
      this.$refs["ethgrpEditform"].validate((valid) => {
        if (valid) {
