              ...
              <el-col :span='24'>
                <el-form 
                  ...
                  <div style="width:100%;text-align:right">
                    <el-button type='primary' @click="addDnsWhiteList">确 定</el-button>
...
export default {
  ...
  methods: {
    getherChange (val) {
      ...
      this.getDnsWhiteList(params)
    },
    getDnsWhiteList (params) {
      ...
      ControlService.getDnsWhiteList(params).then(res => {
        if (res.errcode === 0) {
          this.load = false
          // let regIp = /^((2[0-4]\d|25[0-5]|[01]?\d\d?)\.){3}(2[0-4]\d|25[0-5]|[01]?\d\d?)$/
      ...
      if (this.selectArr.length < 1) {
        ...
        this.$confirm('此操作将删除当前所选白名单, 是否继续?', '提示', {
          ...
          .then(() => {
            ...
            let params = {8: `${this.query.gether}|${ips}`}
            ControlService.delDnsWhiteList(params)
              .then(res => {
                if (res.errcode === 0) {
                  this.$message({
                    type: 'success',
      ...
      if (this.tableData.length < 1) {
        ...
        this.$confirm('此操作将清空当前所有日志, 是否继续?', '提示', {
          ...
          type: 'warning'
            ...
            .then(() => {
              let params = {8: `${this.query.gether}`}
              ControlService.clearDnsWhiteList(params)
          .then((res) => {
            if (res.errcode === 0) {    
              this.$message({
                type: 'success',
    ...
    addDnsWhiteList () {
      this.$refs['singleForm'].validate((valid) => {
        if (valid) {
          let params = {8: `${this.query.gether}|${this.singleFormData.ip}|${this.singleFormData.desc}`}
          ControlService.addDnsWhiteList(params)
            .then((res) => {
              if (res.errcode === 0) {
                this.$message({
                  type: 'success',
