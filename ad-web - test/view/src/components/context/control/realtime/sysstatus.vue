<template>
  <el-row>
    <el-col :span='24'>
      ...
      <div class="chart">
        <h3>设备内存存储状态</h3>
        <memory :picData.sync='memoryData' :timeRange.sync="timeRange" :status='serverStatus'></memory> 
      </div>
      <div class="chart">
        <h3>设备硬盘存储状态</h3>
        <disk :picData='diskData' :timeRange="timeRange" :status='serverStatus'></disk>  
      </div>
    </el-col>
  </el-row>
</template>
  ...
  methods: {
    getState (index) {
      currentService.getState(index)
        .then((res) => {
          if (res.errcode === 0) {
            let cpuDataUser = res[2].map((item, index) => {
              let obj = {
            ...
            // let cpuDataSys = res[3].map((item, index) => {
            //   let obj = {
            ...
            //   return obj
            // })
            this.cpuData = cpuDataUser
            // this.cpuData = cpuDataUser.concat(cpuDataSys)
            
            this.memoryData = res[4].map((item, index) => {
              let obj = {
